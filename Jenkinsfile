pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'MOSQUITTO_VERSION'
    IG_USER = 'imagegenius'
    IG_REPO = 'docker-mosquitto'
    CONTAINER_NAME = 'mosquitto'
    DOCKERHUB_IMAGE = 'imagegenius/mosquitto'
    DEV_DOCKERHUB_IMAGE = 'igdev/mosquitto'
    PR_DOCKERHUB_IMAGE = 'igpipepr/mosquitto'
    DIST_IMAGE = 'alpine'
    DIST_TAG = '3.16'
    DIST_REPO = 'http://dl-cdn.alpinelinux.org/alpine/v3.16/main/'
    DIST_REPO_PACKAGES = 'mosquitto'
    MULTIARCH='false'
    CI='false'
    CI_WEB='false'
    CI_PORT='1883'
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/imagegenius/docker-mosquitto/packages\" \
          --label \"org.opencontainers.image.source=https://github.com/imagegenius/docker-mosquitto\" \
          ...
          --label \"org.opencontainers.image.description=[Mosquitto](https://mosquitto.org/) an open source (EPL/EDL licensed) message broker that implements the MQTT protocol versions 5.0, 3.1.1 and 3.1. Mosquitto is lightweight and is suitable for use on all devices from low power single board computers to full servers.\" \
